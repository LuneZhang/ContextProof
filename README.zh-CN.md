...
ContextProof 是一个可安装到 coding agent 中的轻量 skill，面向
`AGENTS.md`、`CLAUDE.md`、`.cursor/rules`、`SKILL.md`、MCP notes、
已保存的 `/init` 仓库说明等 agent-facing context。它不是通用 Markdown
linter。
...
context-proof skill 为当前仓库准备 workflow、审查并优化 agent context。
候选稿必须写入 .contextproof/candidates/，和原文对比，不要覆盖 AGENTS.md、
CLAUDE.md、.cursor/rules、SKILL.md 或其他源 context 文件。
```
...
支持的安装范围：`agents`、`codex`、`claude`、`opencode`、
`project-agents`、`project-claude`、`project-opencode`。安装脚本只复制
`skill/context-proof`，不会修改你已有的 agent context 文件。

## 它做什么

ContextProof 帮助 coding agent 优化持久化上下文，但不会自动替换源文件：

1. `prepare-workflow`：发现 agent context，审查问题，判断场景，选择优化模板，并写出 `.contextproof/workflow.md`。
2. coding agent 根据 workflow 在 `.contextproof/candidates/` 下写候选稿。
3. `review-candidate`：检查阻断项、分数变化、token 变化、保留项和回归风险。
4. 用户查看报告后手动决定是否采用候选稿。
...
skill 是主要产品形态。CLI 是 skill 背后的 deterministic runner，也是在无法直接加载 skill 时的 fallback。
...
| `SKILL.md`、MCP notes、已保存的 `/init` 说明 | 未保存成 context 文件的原始 `/init` 输出 |

ContextProof 不会自动运行 Codex、Claude Code、OpenCode、Cursor、Gemini、
Copilot 或 Pi。它审查本地文件并写本地报告。
...
- 它指导当前 coding agent 写候选稿，不自动覆盖源文件。
- 它使用 deterministic checks，不使用 LLM judge。
- Gold reference、scorer calibration、acceptance test 是维护者工具，不是普通用户主流程。

详见 [Capability Boundaries](docs/CAPABILITY_BOUNDARIES.md) 和
[Product Strategy](docs/PRODUCT_STRATEGY.md)。

## 维护者工作流

内置 fixtures 和 calibration 用于改进 ContextProof 自身，不是普通用户必需步骤。
